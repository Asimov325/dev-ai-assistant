...
public class ConfigurationController {
    ...
    @PostMapping("/settings/confluence/test") public String testConfluence(@RequestParam String url,@RequestParam String username,@RequestParam(required=false)String token,@RequestParam String space,Model model){IntegrationConfig current=configuration.confluence();IntegrationConfig value=new IntegrationConfig(url,username,secret(token,current),space);ConnectionResult result=atlassian.testConfluence(value);if(result.success())pendingConfluence=value;else pendingConfluence=null;message(model,"confluence",result);model.addAttribute("confluenceSpaceDraft",space);populate(model);return "settings";}
