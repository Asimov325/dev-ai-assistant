...
public class ConfigurationController {
    ...
    @PostMapping("/settings/git/check") public String checkGit(@RequestParam String source,@RequestParam String location,@RequestParam(required=false)String username,@RequestParam(required=false)String token,Model model){try{GitSourceInfo info=inspectGit(source,location,username,token);pendingGit=new ConfiguredGitRepository(info.sourceName(),source.toUpperCase(),location,username,token,info.branches());model.addAttribute("gitMessage","Conexión Git válida. Se detectaron "+info.branches().size()+" ramas. Ya puedes guardar.");}catch(RuntimeException ex){pendingGit=null;model.addAttribute("gitError",ex.getMessage());}populate(model);return "settings";}
    @PostMapping("/settings/git/save") public String saveGit(Model model){if(pendingGit==null)model.addAttribute("gitError","Primero debes probar la conexión Git.");else{configuration.saveRepository(pendingGit);pendingGit=null;model.addAttribute("gitMessage","Configuración Git guardada.");}populate(model);return "settings";}
    ...
    @PostMapping("/settings/jira/save") public String saveJira(Model model){if(pendingJira==null)model.addAttribute("jiraError","Primero debes probar la conexión Jira.");else{configuration.saveJira(pendingJira);pendingJira=null;model.addAttribute("jiraMessage","Configuración Jira guardada.");}populate(model);return "settings";}
    ...
    @PostMapping("/settings/confluence/test") public String testConfluence(@RequestParam String url,@RequestParam String username,@RequestParam(required=false)String token,@RequestParam String space,@RequestParam(defaultValue="false")boolean shared,Model model){IntegrationConfig base=shared?configuration.jira():configuration.confluence();IntegrationConfig value=shared&&base!=null?new IntegrationConfig(base.url(),base.username(),base.secret(),space):new IntegrationConfig(url,username,secret(token,base),space);ConnectionResult result=atlassian.testConfluence(value);if(result.success())pendingConfluence=value;else pendingConfluence=null;message(model,"confluence",result);populate(model);return "settings";}
    @PostMapping("/settings/confluence/save") public String saveConfluence(Model model){if(pendingConfluence==null)model.addAttribute("confluenceError","Primero debes probar la conexión Confluence.");else{configuration.saveConfluence(pendingConfluence);pendingConfluence=null;model.addAttribute("confluenceMessage","Configuración Confluence guardada.");}populate(model);return "settings";}
    ...
    @PostMapping("/settings/ai/test") public String testAi(@RequestParam(required=false)String token,Model model){String key=secret(token,configuration.ai());try{gemini.useRuntimeApiKey(key);aiService.generate("Responde únicamente: conexión correcta");pendingAi=new IntegrationConfig("gemini",aiService.providerId(),key,"gemini");model.addAttribute("aiMessage","Conexión con "+aiService.providerId()+" correcta. Ya puedes guardar.");}catch(RuntimeException ex){pendingAi=null;IntegrationConfig saved=configuration.ai();gemini.useRuntimeApiKey(saved==null?null:saved.secret());model.addAttribute("aiError",ex.getMessage());}populate(model);return "settings";}
    @PostMapping("/settings/ai/save") public String saveAi(Model model){if(pendingAi==null)model.addAttribute("aiError","Primero debes probar la conexión de IA.");else{configuration.saveAi(pendingAi);gemini.useRuntimeApiKey(pendingAi.secret());pendingAi=null;model.addAttribute("aiMessage","Configuración de IA guardada.");}populate(model);return "settings";}
