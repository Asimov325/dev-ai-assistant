...
public class DocumentationGenerationService {
    ...
    private void appendDpcStructure(StringBuilder prompt) {
        prompt.append("PLANTILLA CORPORATIVA DPC. RESPETA ESTA ESTRUCTURA Y TIPO DE PRESENTACIÓN:\n\n")
                ...
                .append("|---:|---|---|---|---|---|\n")
                .append("| 1 | [actividad/recurso de pase] | [Nuevo/Modificación/etc.] | [evidencia o ").append(VALIDATION_PLACEHOLDER).append("] | [evidencia o ").append(VALIDATION_PLACEHOLDER).append("] | [evidencia o ").append(VALIDATION_PLACEHOLDER).append("] |\n\n")
                .append("REGLAS OBLIGATORIAS PARA PROCEDIMIENTO DEL PASE:\n")
                .append("- El procedimiento representa ACTIVIDADES DE PASE ordenadas; no es un inventario de archivos modificados.\n")
                .append("- Agrupa todos los archivos Java/fuentes que pertenecen al mismo despliegue en UN SOLO paso de Fuentes/Aplicación. Nunca crees una fila por cada clase Java salvo evidencia explícita de despliegues independientes.\n")
                .append("- Si existen scripts de creación/reversión evidenciados, consolídalos como un paso de Base de Datos y conserva su orden relativo de ejecución cuando pueda demostrarse.\n")
                .append("- Si existen cambios de fuentes evidenciados, crea un paso de Fuentes/Branch usando la rama del requerimiento indicada en EVIDENCIA GIT. Puede describirse la obtención de las fuentes y la necesidad de compilar/desplegar, pero NO inventes comandos, herramienta de build, servidor, pipeline, ambiente ni parámetros. Lo no demostrado va como ").append(VALIDATION_PLACEHOLDER).append(".\n")
                .append("- Si Git/Jira evidencia una nueva opción, LDAP/LDIF o configuración equivalente, crea un paso independiente para esa actividad. Si además evidencia asociación a perfiles, crea otro paso independiente para dicha asociación.\n")
                .append("- Scripts MQ, configuraciones u otros recursos se convierten en pasos solo cuando exista evidencia y deben ubicarse según la dependencia técnica demostrable.\n")
                .append("- El orden depende del requerimiento. No fuerces siempre BD=1, Fuentes=2, Opción=3, Perfiles=4; usa ese patrón solo cuando la evidencia y dependencias lo sustenten.\n")
                .append("- Si sabes que una actividad existe pero no conoces su instrucción exacta de ejecución, validación o reversión, conserva la actividad y coloca ").append(VALIDATION_PLACEHOLDER).append(" únicamente en las celdas desconocidas.\n")
                .append("- No copies comandos, rutas LDAP, CN, perfiles ni instrucciones de documentos de referencia; solo utiliza la evidencia Jira/Git de este requerimiento.\n\n")
